...
import { default as orders_data } from '../../data/e-commerce/orders';
...
const Orders2 = () => {
  ...
  const actionFormatter = (dataField, { id }: row) => (
    ...
    <UncontrolledDropdown>
      ...
      <DropdownMenu right className="border py-2">
        ...
        <DropdownItem onClick={() => {console.log('ddd'); setOrders(orders.filter(order => order.id !== id));}} className="text-danger">
          Delete
        </DropdownItem>
      </DropdownMenu>
    </UncontrolledDropdown>
  ...
  return (
    <Card className="mb-3">
      <FalconCardHeader title="Orders2" light={false}>
        {isSelected ? (
